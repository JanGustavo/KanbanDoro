...
4. Na raiz, compile com `KANBANDORO_GOOGLE_CLIENT_ID='ID.apps.googleusercontent.com' KANBANDORO_API_URL='http://127.0.0.1:8000' npm run build`. Recarregue a pasta `dist` no navegador. Para outros computadores, use uma **origem HTTPS** acessível à extensão e ao backend; `http://127.0.0.1` só alcança um servidor no mesmo computador.
