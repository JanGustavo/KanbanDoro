...
const chrome = {
  ...
  runtime: { getURL: path => path, onInstalled: { addListener() {} }, onStartup: { addListener() {} }, onMessage: { addListener(fn) { listeners.message = fn; } } },
  storage: { local: { setAccessLevel: async () => {}, get: async () => ({ session: { phase: 'running', endsAt: Date.now() + 60_000 } }) }, onChanged: { addListener() {} } },
