...
O quadro da extensão ainda usa armazenamento local. A API atende as Connections Google com troca OAuth, renovação de token e consultas sob demanda; não sincroniza as tarefas nem chama um provedor de IA. Não coloque a chave de IA no cadastro: a chave fica no navegador até existir um fluxo de integração explícito e seguro.
